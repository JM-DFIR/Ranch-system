...
// The Vet Visits register (session-pack.md Part 5 — "M3 remainder").
// One row per visit, not per animal — a visit can cover several
// animals at once (vet_visit_animals), listed in the Animals column.
export function VetVisitRegisterPage() {
  const { profile } = useAuth();
  const { ranch } = AuthenticatedRoute.useSearch();
  const search = VetVisitsRoute.useSearch();
  const navigate = VetVisitsRoute.useNavigate();

  const page = search.page ?? 0;
  const pageSize = search.pageSize ?? DEFAULT_PAGE_SIZE;
  const pagination: PaginationState = { pageIndex: page, pageSize };

  const { data, isLoading, isError, error, refetch } = useVetVisitRegister(profile?.orgId, {
    ranchId: ranch,
  ...
  const emptyState = (
    <EmptyState
      icon={Stethoscope}
      title="No vet visits recorded yet"
      description="Vet visits recorded from an animal's profile, the register, or the dashboard's quick actions all show up here."
    />
  );

  return (
    <div className="flex flex-col gap-4">
      <PageHeader title="Vet visits" description={totalCount > 0 ? `${totalCount.toLocaleString()} vet visits recorded` : undefined} />
      <div className="overflow-hidden rounded-card border border-line">
        {isError ? (
          <div className="p-4">
            <ErrorState
              title="Couldn't load vet visits"
          ...
          <>
            ...
            />
            <DataTable
              columns={vetVisitRegisterColumns}
              data={rows}
              rowCount={totalCount}
              isLoading={isLoading}
              sorting={NO_SORTING}
